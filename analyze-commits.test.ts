...
Deno.test("getNextReleaseVersion initializes version correctly", async () => {
  const commits: GitHubCommit[] = [{ sha: "", commit: { message: "feat: initial commit" } }];
...
Deno.test("getNextReleaseVersion bumps major version correctly", async () => {
  const commits: GitHubCommit[] = [{ sha: "", commit: { message: "feat!: add new authentication system" } }];
...
Deno.test("getNextReleaseVersion bumps minor version correctly", async () => {
  const commits: GitHubCommit[] = [{ sha: "", commit: { message: "feat: add user profile page" } }];
  const result = await getNextReleaseVersion({ commits, lastReleaseVersion: "1.2.3" });
  assertEquals(result, '1.3.0');
});

Deno.test("getNextReleaseVersion bumps patch version correctly", async () => {
  const commits: GitHubCommit[] = [{ sha: "", commit: { message: "fix: fix login bug" } }];
  const result = await getNextReleaseVersion({ commits, lastReleaseVersion: "1.2.3" });
